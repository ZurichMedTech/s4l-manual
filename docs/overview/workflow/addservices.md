## Adding, Deleting and Updating Services
...
4. In the `Project Window` click the `Services Updates` button. If the `Services Updates` button is deactivated (greyed out), close the `Project Window` by clicking the `x` in the top right corner and go back to step 3 to select a different project.
